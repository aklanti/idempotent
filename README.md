...
| either | different | `FingerprintMismatch`: answer 400, the key was reused for another request |
...
- The processing lease is how long a claim survives a worker that died holding it; a minute is typical. `keep_alive` renews it while the side effect runs, and `ClaimGuard::touch` extends it by hand.
